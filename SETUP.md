...
(Run this from inside `meeting_notifyer/` — `pyenv local` is already set to
3.12.10 for this folder.)
...
- The app is Windows-only (uses `winsound` for the alert sound and Windows
  Startup/Start Menu folders for autostart) — it won't run as-is on
  macOS/Linux.
